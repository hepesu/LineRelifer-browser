...
const MODEL_PATH =
  process.env.NODE_ENV === "production"
    ? "https://hepesu.github.io/LineRelifer-browser/assets/model_180317/model.json"
    : "./assets/model_180317/model.json";
...
export default {
  ...
  methods: {
    ...
    generatePatches() {
      ...
      for (let i0Index = 0; i0Index < i0List.length; i0Index++) {
        ...
        for (let j0Index = 0; j0Index < j0List.length; j0Index++) {
          ...
          inputProcess(patch);
          const buffer = tf.buffer([size, size], "float32", patch.data);

          patches.push(buffer.toTensor());
        }
      }
    ...
    async runPredict() {
      ...
      const outputPatches = await Promise.mapSeries(
        inputPatches,
        async patch => {
          const outputData = await this.model
            .predict(patch.reshape([1, this.patchSize, this.patchSize, 1]))
            .data();

<template>
    <div>
        ...
        <button v-on:click="runPredict">Run</button>
        <div>
...
export default {
    ...
    methods: {
        handleLoadingProgress(progress) {
            ...
            console.log("Model Loading ", this.modelLoadingProgress);
            if (progress === 100) {
        ...
        handleInitProgress(progress) {
            ...
            console.log("Model Initializing ", this.modelInitProgress);
            if (progress === 100) {
        ...
        generatePatches() {
            ...
            const grayTensor = ndarray(new Float32Array(height * width * 1), [height, width, 1]);
            ops.assign(grayTensor.pick(null, null, 0), dataTensor.pick(null, null, 0));
            ...
            for (let i0Index = 0; i0Index < i0List.length; i0Index++) {
                ...
                for (let j0Index = 0; j0Index < j0List.length; j0Index++) {
                    ...
                    const patch = ndarray(new Float32Array(size * size * 1), [size, size, 1]);
                    const i1 = i0 + size > height ? height : i0 + size;
                    const j1 = j0 + size > width ? width : j0 + size;
                    ops.assigns(patch.pick(null, null, 0), 255);
                    ops.assign(
                        patch.hi(i1 - i0, j1 - j0, 0).lo(0, 0, 0).pick(null, null, 0),
                        grayTensor.hi(i1, j1, 0).lo(i0, j0, 0).pick(null, null, 0)
                    );
                    inputProcess(patch);
        ...
        combinePatches(patches) {
            ...
            const combined = ndarray(new Float32Array(this.imgHeight * this.imgWidth * 1), combinedShape);
            ops.assigns(combined.pick(null, null, 0), 0);

            const denorm = ndarray(new Float32Array(this.imgHeight * this.imgWidth * 1), combinedShape);
            ops.assigns(denorm.pick(null, null, 0), 0.00001);

            const patchOnes = ndarray(new Float32Array(size * size * 1), [size, size, 1]);
            ...
            for (let n = 0, len = patches.length; n < len; n++) {
                ...
                const patchHeight = i0 + size > this.imgHeight ? this.imgHeight - i0 : size;
                const patchWidth = j0 + size > this.imgWidth ? this.imgWidth - j0 : size;

                let mask = this.masks.center;

                if (j0 == j0List[0] && i0 == i0List[0])
                    mask = this.masks.leftTopCorner;
                ...
                else if (j0 == j0List[j0List.length - 1] && i0 == i0List[i0List.length - 1])
                    mask = this.masks.rightBottomCorner;
                else if (j0 == j0List[0])
                    mask = this.masks.leftBorder;
                else if (j0 == j0List[j0List.length - 1])
                    mask = this.masks.rightBorder;
                else if (i0 == i0List[0])
                    mask = this.masks.topBorder;
                else if (i0 == i0List[i0List.length - 1])
                    mask = this.masks.bottomBorder;

                over(
                    combined.hi(i0 + patchHeight, j0 + patchWidth, 0).lo(i0, j0, 0).pick(null, null, 0),
                    patches[n].hi(patchHeight, patchWidth, 0).lo(0, 0, 0).pick(null, null, 0),
                    mask.hi(patchHeight, patchWidth, 0).lo(0, 0, 0)
                );

                over(
                    denorm.hi(i0 + patchHeight, j0 + patchWidth, 0).lo(i0, j0, 0).pick(null, null, 0),
                    patchOnes.hi(patchHeight, patchWidth, 0).lo(0, 0, 0).pick(null, null, 0),
                    mask.hi(patchHeight, patchWidth, 0).lo(0, 0, 0)
                );
            }
            ...
            contrastStretch(combined.pick(null, null, 0), 80, 220);

            // assign channels
            const outShape = [this.imgHeight, this.imgWidth, 4];
            const output = ndarray(new Float32Array(this.imgHeight * this.imgWidth * 4), outShape);
        ...
        async runPredict() {
            ...
            const outputPatches = await Promise.mapSeries(inputPatches, async patch => {
                ...
                return ndarray(outputData[outputName], [this.patchSize, this.patchSize, 1]);
            });
